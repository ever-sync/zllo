...
export function ShopNotificacoesClient({ initial }: { initial: NotificationRow[] }) {
  ...
  const onOpen = async (n: NotificationRow) => {
    ...
    const href = notificationHref(n, 'assistencia');
    ...
    <>
      <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-g600">Solicitações, vendas e mensagens da sua loja.</p>
        </div>
      ...
      {rows.length === 0 ? (
        <div className="rounded-[14px] border border-dashed border-line bg-white p-8 text-center">
          ...
          <p className="mx-auto mt-1 max-w-md text-sm text-g600">
            Quando receber solicitações ou vendas, elas aparecerão aqui.
          </p>
          <Link href="/operacao" className="mt-5 inline-flex text-sm font-semibold text-blue">
            Voltar ao painel
          </Link>
        </div>
      ) : (
