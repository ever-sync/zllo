...
/** Carrinho web — uma loja por vez, persistido em localStorage. */
...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  /* eslint-disable react-hooks/set-state-in-effect -- hydrate cart from localStorage on mount */
  useEffect(() => {
    try {
  ...
  useEffect(() => {
    if (!hydrated) return;
    localStorage.setItem(KEY, JSON.stringify({ shopId, shopName, items }));
  ...
  const value = useMemo<CartState>(() => {
    ...
    return { shopId, shopName, items, count, total, hydrated, add, replaceWith, setQty, remove, clear };
  }, [shopId, shopName, items, hydrated]);
