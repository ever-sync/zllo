...
export default function ShopNotificacoes() {
  ...
  return (
    <Screen background={colors.canvas}>
      <AppHeader title="Notificações" subtitle="Solicitações, vendas e mensagens" />
      ...
      {loadError ? (
        ...
        <EmptyState
          ...
          description="Quando receber solicitações ou vendas, elas aparecerão aqui."
          actionLabel="Voltar ao painel"
          onAction={() => router.replace('/(shop)/(tabs)')}
        />
      ) : (
        <View style={{ gap: 10, marginTop: 8 }}>
          {rows.map((n) => {
