...
process.stdin.on('end', () => {
  ...
  if (j.status === 'FINISHED') {
    console.log('');
    console.log('Smoke test (docs/EAS_BUILD.md):');
    console.log('  1. eas build:run -p', j.platform?.toLowerCase(), '--profile', j.buildProfile);
    console.log('  2. Login cliente + assistência');
    console.log('  3. Push token em push_tokens (Supabase)');
    console.log('  4. Solicitar reparo → orçamento → Pix');
  }
});
"
